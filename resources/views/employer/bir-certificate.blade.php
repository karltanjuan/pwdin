            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="input-group">
                        <input type="file" class="form-control form-control-lg bir_certificate" id="bir_certificate"
                    ...
                    @php
                        $bir_certificate = str_replace(
                            'public',
                            'storage',
                            auth()
                                ->guard('employers')
                                ->user()->bir_certificate,
                        );
                        $path = pathinfo($bir_certificate);
                    @endphp

                    @if (isset($path['extension']) && $path['extension'] == 'pdf')
                        <a href="{{ asset($bir_certificate) }}" target="_blank" type="button"
                            class="btn btn-outline-secondary btn-lg"
                            style="padding-left: 2.5rem; padding-right: 2.5rem;">View BIR certificate</a>
                    @endif

                    @if (isset($path['extension']) && in_array($path['extension'], ['png', 'jpg', 'jpeg']))
                        <img class="rounded border w-100 img-fluid img-preview" src="{{ asset($bir_certificate) }}"
                            alt="BIR Certificate">
                    @endif
